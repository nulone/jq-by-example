...
Run all: jq-by-example --task all
```
...
After installation:
  1. Verify: jq --version
  2. Try again: jq-by-example --help
...
View all: jq-by-example --list-tasks
```
...
### Приоритет 2 (Средние):
4. **Configuration file** - ~/.jq-by-example/config.yaml (MEDIUM effort)
...
```bash
# ✅ Colorized output
jq-by-example --list-tasks  # Работает с цветами

# ✅ List tasks
jq-by-example --list-tasks  # Показывает все задачи с группировкой

# ✅ Fuzzy matching
jq-by-example --task nested-fields  # Предлагает "nested-field"

# ✅ JSON validation
jq-by-example -i '{"x": 1' -o '1'  # Показывает helpful error
